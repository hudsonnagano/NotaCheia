...
<button className="btn btn-red" onClick={() => { setSavedNome(nome.trim() || "Anônimo"); setSavedWpp(wpp.trim()); setStep("prize"); }} disabled={!nome.trim()}>
          {nome.trim() ? "Resgatar meu prêmio! 🎁" : "Digite seu nome para continuar"}
        </button>
      </div>
    </div>
  );
...
function CardVisita({ f, compact = false, setClienteModal, calcMediaEstrelas }) {
    ...
    <>
      ...
      {clienteVista === "agrupado" && (
        grupos.length === 0
          ? <div style={{textAlign:"center",color:"var(--muted)",padding:40,fontSize:14}}>
              ...
              <span style={{fontSize:12}}>Os clientes aparecem aqui após responderem o questionário.</span>
            </div>
          : grupos.map((g, i) => (
            <div key={i} style={{ background:"var(--d1)", border:"1px solid var(--border)", borderRadius:14, padding:14, marginBottom:10 }}>
              ...
              {g.visitas.map((f, j) => <CardVisita key={j} f={f} compact setClienteModal={setClienteModal} calcMediaEstrelas={calcMediaEstrelas} />)}
            </div>
          ))
      )}
      ...
      {clienteVista === "lista" && (
        fbsFiltrados.length === 0
          ? <div style={{textAlign:"center",color:"var(--muted)",padding:40,fontSize:14}}>Nenhum registro encontrado.</div>
          : fbsFiltrados.map((f, i) => <CardVisita key={i} f={f} compact={false} setClienteModal={setClienteModal} calcMediaEstrelas={calcMediaEstrelas} />)
      )}
        ...
        <div className="modal-bg" onClick={() => setClienteModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxHeight:"85vh", overflowY:"auto" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
              <div>
                <div style={{ fontFamily:"var(--ff-head)", fontSize:18 }}>👤 {clienteModal.nome || "Anônimo"}</div>
                <div style={{ fontSize:12, color:"var(--muted)", marginTop:3 }}>🗓️ {clienteModal.data || "—"}</div>
                {calcMediaEstrelas(clienteModal) && <div style={{ fontSize:13, color:"var(--ac)", fontWeight:700, marginTop:3 }}>⭐ {calcMediaEstrelas(clienteModal)} média</div>}
              </div>
            ...
            <div className="div" />
            {est.questions.map(q => {
              const resp = clienteModal.answers?.[q.id];
              if (resp === undefined || resp === null || resp === "") return null;
              return (
                <div key={q.id} style={{ padding:"10px 0", borderBottom:"1px solid var(--border)" }}>
                  <div style={{ fontSize:11, color:"var(--muted)", fontWeight:700, marginBottom:5 }}>{TYPE_ICON[q.type]} {q.label}</div>
                  {q.type === "stars" && (
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <div style={{ display:"flex", gap:2 }}>{[1,2,3,4,5].map(s => <span key={s} style={{ fontSize:16, filter: s <= resp ? "none" : "grayscale(1) opacity(0.2)" }}>⭐</span>)}</div>
                      <span style={{ fontSize:12, fontWeight:800, color: resp >= 4 ? "var(--green)" : resp >= 3 ? "var(--yellow)" : "var(--red)" }}>{["","Ruim","Regular","Bom","Ótimo","Excelente"][resp]}</span>
                    </div>
                  )}
                  {q.type === "nps" && (
                  ...
                  {(q.type === "choice" || q.type === "staff") && (
                    <span style={{ background:"var(--d3)", border:"1px solid var(--border)", borderRadius:20, padding:"4px 12px", fontSize:13, fontWeight:700 }}>{String(resp).replace("Outro:","").trim()}</span>
                  )}
                  {q.type === "text" && (
                    <div style={{ background:"var(--dark)", borderLeft:"3px solid var(--ac)44", borderRadius:"0 8px 8px 0", padding:"8px 12px", fontSize:13, fontStyle:"italic", color:"#bbb" }}>"{resp}"</div>
                  )}
                </div>
              );
            })}
            {clienteModal.premio && (
              <div style={{ marginTop:14, display:"flex", alignItems:"center", gap:8 }}>
                ...
                <span style={{ background:"var(--d2)", border:"1px solid var(--ac)33", borderRadius:20, padding:"4px 12px", fontSize:13, fontWeight:700 }}>{clienteModal.premio}</span>
              </div>
            )}
            {(clienteModal.answers?.wpp || clienteModal.whatsapp) && (
              <a href={`https://wa.me/55${(clienteModal.answers?.wpp || clienteModal.whatsapp || "").replace(/\D/g,"")}`}
  ...
  return (
    <div className="shell">
      ...
      <div className="main">
       ...
       {tab === "clientes" && <AbaClientes est={est} />}
        {tab === "relatorio" && (() => {
          const fbSemana = getFeedbacksSemana(0);
          const fbAnterior = getFeedbacksSemana(1);
          const sqs = est.questions.filter(q => q.type === "stars");
...
function MasterPanel({ establishments, setEstablishments, onLogout }) {
  ...
  return (
    <div className="shell">
      ...
      {/* Modal ver feedbacks */}
      {viewEst && (<div className="modal-bg" onClick={() => setViewEst(null)}><div className="modal" onClick={e => e.stopPropagation()}><div className="modal-title">{viewEst.emoji} {viewEst.name}</div>{viewEst.feedbacks.length === 0 && <div style={{ color: "var(--muted)" }}>Nenhum feedback ainda.</div>}{(() => { const npsQ = viewEst.questions.find(q => q.type === "nps"); const staffQ = viewEst.questions.find(q => q.type === "staff"); const textQ = viewEst.questions.find(q => q.type === "text"); return viewEst.feedbacks.map((f, i) => (<div className="fb" key={i} style={{ marginBottom: 8 }}><div className="fb-top"><div className="fb-name">👤 {f.nome}</div><div className="fb-date">{f.data || "Agora"}</div></div><div style={{ fontSize: 12, color: "var(--muted2)" }}>NPS: {npsQ ? (f.answers?.[npsQ.id] ?? "-") : "-"} · Atendente: {staffQ ? (f.answers?.[staffQ.id] ?? "-") : "-"}</div>{textQ && f.answers?.[textQ.id] && <div className="fb-comment">💬 "{f.answers[textQ.id]}"</div>}{f.premio && <div className="fb-prize">🎁 {f.premio}</div>}</div>)); })()}<button className="btn btn-ghost" style={{ marginTop: 14 }} onClick={() => setViewEst(null)}>Fechar</button></div></div>)}
